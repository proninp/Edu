...
namespace CodeWars._6kyu
{
     ...
     * Each book has a code c of 3, 4, 5 or more capitals letters. The 1st letter of a code is the capital letter of the book category.
    ...
    public class HelpTheBookseller
    {
        ...
        {
            Dictionary<string, int> dict = lstOf1stLetter.ToDictionary(k => k, v => 0);
            lstOf1stLetter.ToList().ForEach(s => lstOfArt.Where(ss => ss.StartsWith(s)).ToList().ForEach(sss => dict[s] += Int32.Parse(sss.Split(' ')[1])));
            return (lstOfArt.Length == 0 || lstOf1stLetter.Length == 0) ? "" : string.Join(" - ", dict.Select(d => "(" + d.Key + " : " + d.Value + ")"));
        }
    }
}
